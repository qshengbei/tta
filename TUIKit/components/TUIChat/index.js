  ...
  methods: {
    ...
    pullKeysBoards(event) {
      ...
      if (height === 0) {
        ...
        // 键盘弹出时，增加额外的40像素间隔，确保键盘和输入框之间有足够的空间
        setNewInputStyle(height + 40);
      }
      this.setData({
    ...
    // 处理表情弹窗显示/隐藏
    handleEmojiToggled(event) {
      const { visible, height } = event.detail;
      if (visible) {
        // 表情弹窗显示时，调整list-box的高度，减少的高度等于表情弹窗的高度
        // 这样消息列表就会向上滚动，确保最新消息可见
        const listBoxHeight = `calc(100vh - 172rpx - ${height + 100}px)`;
        this.setData({
          listBoxStyle: `height: ${listBoxHeight};`,
        }, () => {
          // 滚动消息列表到底部，确保最新消息可见
          const MessageListEle = this.selectComponent('#MessageList');
          if (MessageListEle && MessageListEle.updateScrollToBottom) {
