...
Page({
  ...
  loadCategories() {
    db.collection('product_types').where({ level: 1 }).get().then(res => {
      ...
      // 默认选择第一个分类
      if (res.data.length > 0 && this.data.activeTab === 'categories') {
        this.selectCategory({ currentTarget: { dataset: { id: res.data[0]._id } } })
      }
    }).catch(err => {
  ...
  goToCategoryProducts(e) {
    const categoryId = e.currentTarget.dataset.id
    wx.navigateTo({
      url: `/pages/admin/product-list/index?type=type&typeId=${categoryId}`
    })
  },
  ...
  goToSeriesProducts(e) {
    const seriesId = e.currentTarget.dataset.id
    wx.navigateTo({
      url: `/pages/admin/product-list/index?type=series&categoryId=${seriesId}`
    })
  },
  ...
  编辑商品(e) {
    const productId = e.currentTarget.dataset.id
    wx.navigateTo({
      url: `/pages/admin/product-publish/index?id=${productId}`
    })
  },
